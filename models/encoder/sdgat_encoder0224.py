...
class SDGATEncoder(object):
    ...
    def encode(self, inputs):
        ...
        h_1 = self.encs
        for _ in range(self.gat_stacks-1):
            attns = []
            for _ in range(self.num_heads):
                attns.append(self.attn_head(h_1, out_sz=head_hidden_dim, num_units=self.hidden_dim, activation=tf.nn.elu, in_drop=0, coef_drop=0))
            h_1 = tf.concat(attns, axis=-1)

        out = []
        for _ in range(self.num_heads):
            out.append(self.attn_head(h_1, out_sz=self.hidden_dim, num_units=self.hidden_dim, activation=tf.nn.elu, in_drop=0, coef_drop=0))
        tf.add_n(out) / self.num_heads
    ...
    def attn_head(self, seq, out_sz, num_units, activation, in_drop=0.0, coef_drop=0.0):
        with tf.name_scope('sdgat_attn'):
            ...
            for _ in range(self.num_stacks-1):
                vals = multihead_attention(vals, out_sz)
