...
class PPO_Agent(object):
    ...
    def __init__(self, config):
        ...
        if config.agent_type == 'PPO1':
            self.beta = tf.Variable(1.0, trainable=False, name="global_step")
            self.d_tar = 0.01
            self.kl_loss_func = tf.keras.losses.KLDivergence()
            self.ppo_type = 1
        elif config.agent_type == 'PPO2':
            self.clip_ratio = 0.01
            self.ppo_type = 2
    ...
    def build_optimization(self):
        ...
        with tf.control_dependencies(update_ops):
            with tf.name_scope('surrogate_update'):
                ...
                #     self.sum_log_pi_old = tf.reduce_sum(self.log_pi_old, axis=[1,2])
                #     self.sum_log_pi_new = tf.reduce_sum(self.log_pi_new, axis=[1,2])
                #     self.ratio = self.sum_log_pi_new - self.sum_log_pi_old
                #     self.ratio_1 = tf.exp(self.ratio)
                #     self.ratio_2 = tf.clip_by_value(self.ratio_1, 1.0-self.clip_ratio, 1.0+self.clip_ratio)
                #     self.surr1 = self.ratio_1 * self.advantage
                #     self.surr2 = self.ratio_2 * self.advantage
                ...
                # self.ratio_1 = tf.exp(self.ratio)
                # self.surr1 = self.ratio_1 * self.advantage
                # self.surr2 = self.advantage + tf.where(self.advantage>0., self.clip_ratio*self.advantage, -self.clip_ratio*self.advantage)
                # self.clipped_surrogate = -tf.reduce_mean(tf.minimum(self.surr1, self.surr2))
                # self.loss1 = self.clipped_surrogate - self.lr1 * tf.reduce_mean(self.entropy_regularization, 0)
                
                # # log_sum version 1
                # self.log_pi_old = self.log_pi_old_
                # self.sum_log_pi_old = tf.reduce_mean(self.log_pi_old, axis=[1,2])
                # self.sum_log_pi_new = tf.reduce_mean(self.log_pi_new, axis=[1,2])
                # self.ratio_1 = self.sum_log_pi_old/self.sum_log_pi_new
                # self.ratio_2 = tf.clip_by_value(self.ratio_1, 1.0-self.clip_ratio, 1.0+self.clip_ratio)
                # self.surr1 = self.ratio_1 * self.advantage
                # self.surr2 = self.ratio_2 * self.advantage
                # self.clipped_surrogate = -tf.reduce_mean(tf.minimum(self.surr1, self.surr2))
                ...
                # # all restricting version 1
                # if self.ppo_type == 2:
                #     self.log_pi_old = self.log_pi_old_
                #     self.ratio_1 = tf.exp(self.log_pi_old-self.log_pi_new)
                #     self.ratio_2 = tf.clip_by_value(self.ratio_1, 1.0-self.clip_ratio, 1.0+self.clip_ratio)
                #     self.adv_mat = tf.tile(self.advantage[:, tf.newaxis, tf.newaxis], tf.constant([1,self.max_length,self.max_length]))
                #     self.surr1 = self.ratio_1 * self.adv_mat
                #     self.surr2 = self.ratio_2 * self.adv_mat
                #     self.clipped_surrogate = -tf.reduce_mean(tf.minimum(self.surr1, self.surr2))
                #     self.loss1 = self.clipped_surrogate - self.lr1 * tf.reduce_mean(self.entropy_regularization, 0)
                
                # # all restricting version 2
                # if self.ppo_type == 2:
                #     self.log_pi_old = self.log_pi_old_
                #     self.ratio_1 = tf.exp(self.log_pi_old-self.log_pi_new)
                #     self.ratio_2 = tf.clip_by_value(self.ratio_1, 1.0-self.clip_ratio, 1.0+self.clip_ratio)
                #     self.surr1 = tf.reduce_mean(self.ratio_1, axis=[1,2]) * self.advantage
                #     self.surr2 = tf.reduce_mean(self.ratio_2, axis=[1,2]) * self.advantage
                #     self.clipped_surrogate = -tf.reduce_mean(tf.minimum(self.surr1, self.surr2))
                #     self.loss1 = self.clipped_surrogate - self.lr1 * tf.reduce_mean(self.entropy_regularization, 0)
                
                # all restricting version 3
                if self.ppo_type == 2:
                    self.log_pi_old = self.log_pi_old_
                ...
                if self.ppo_type == 1:
                    self.log_pi_old = self.log_pi_old_
                    self.log_gap = self.log_pi_new - self.log_pi_old
                    self.ratio = tf.exp(tf.reduce_sum(self.log_gap, axis=[1,2]))
                    self.surr = self.ratio * self.advantage
                    self.kl_loss = tf.reduce_mean(tf.exp(self.log_pi_new) * self.log_gap, axis=[1,2])
                    self.kl_penalty = -tf.reduce_mean(self.surr - self.beta * self.kl_loss)
                    self.loss1 = self.kl_penalty - self.lr1 * tf.reduce_mean(self.entropy_regularization, 0)
