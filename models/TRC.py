    ...
    def build_optimization(self):
        ...
        with tf.control_dependencies(update_ops):
            with tf.name_scope('surrogate_update'):
                ...
                # rollback
                self.pi_old = tf.clip_by_value(tf.exp(self.log_pi_old_), 0.00001, 0.99999)
                self.pi_new = tf.clip_by_value(tf.exp(self.log_pi_new), 0.00001, 0.99999)
                self.log1 = self.log_pi_new - self.log_pi_old_
                # self.log1 = self.log_pi_old_ - self.log_pi_new
                self.log2 = tf.log(1-self.pi_old) - tf.log(1-self.pi_new)
                self.kl_mat = -self.pi_old*self.log1 + (1-self.pi_old)*self.log2
                # self.log2 = tf.log(1-self.pi_new) - tf.log(1-self.pi_old)
                # self.kl_mat = self.pi_new*self.log1 + (1-self.pi_new)*self.log2
                # self.ratio = tf.exp(self.log1)
                self.cliped = tf.clip_by_value(self.log1, log(1-self.clip_ratio), log(1+self.clip_ratio))
                self.trgcliped = tf.where(self.kl_mat>=self.sigma, self.cliped, self.log1)
                self.trgratio = tf.exp(tf.reduce_sum(self.trgcliped, axis=[1,2]))
                self.surr =  self.trgratio * self.advantage
                self.clipped_surrogate = -tf.reduce_mean(self.surr)
